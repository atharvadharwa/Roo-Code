	...
	async *createMessage(request: ApiRequest): AsyncGenerator<ApiStreamChunk> {
		...
		try {
			...
			if (json.choices && json.choices.length > 0) {
				// Try multiple content locations in non-streaming response
				const content =
					json.choices[0]?.message?.content ||
					json.choices[0]?.text ||
					json.choices[0]?.content ||
					json.choices[0]?.response ||
					json.choices[0]?.output
				...
				if (content) {
					...
					this.outputChannel.appendLine("Choice structure: " + JSON.stringify(json.choices[0], null, 2))
				}
			} else {
				// If no choices, look for content at top level
				const content = json.message?.content || json.text || json.content || json.response || json.output
				...
				if (content) {
					...
					this.outputChannel.appendLine("No choices or top-level content found in non-streaming response")
				}
			}
		} catch (e) {
